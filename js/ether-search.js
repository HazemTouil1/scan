function etherSearchInit({ id, isMainPanel = true, classResultWrapper, groupIncludes = [], attributes }) {
    ...
    function triggerScrollAction() {
        ...
        if (searchInput.value === '' && searchInput === document.activeElement && mainContainer.querySelector('.auto-results-wrapper').classList.contains('auto-is-active')) {
            ...
            mainContainer.querySelector(`#auto-${id}-results`).addEventListener("scroll", function (e) {
                ...
                if (currentActiveSearchCategories !== currentActiveSearchCategoriesLoop) {
                    if (isResetActiveScrollOverride === false)
                        resetActiveClass(mainContainer.querySelector(`.${currentActiveSearchCategoriesLoop.replace('search-category', 'search-tag').replace('-(erc-20)', '').replace('-(erc-721-&-1155)', '')}`));
    ...
    if (isMainPanel || isAdvancedFilterSearch) {
        window.addEventListener("keydown", (event) => {
            ...
            if ((event.code === 'ArrowDown' || event.keyCode === 40) || (event.code === 'ArrowUp' || event.keyCode === 38)) {
                if (mainContainer.querySelector(`#auto-${id}-results`).classList.contains('d-none') && searchInput === document.activeElement && mainContainer.querySelector('.auto-results-wrapper').classList.contains('auto-is-active')) {
                    searchInput.value = '';
                }
            }
    ...
    mainContainer.addEventListener("click", function (e) {
        if (e.target.classList.contains('search-panel-tags')) {
            mainContainer.querySelector(`#auto-${id}-results`).scrollTo({
                top: mainContainer.querySelector('.search-category-' + e.target.textContent.toLowerCase().replace(/ /g, '-').replace('-(erc-20)', '').replace('-(erc-721-&-1155)', '')).offsetTop + 1,
                left: 0,
                behavior: 'smooth'
            });
    ...
    let autocomplete = new Autocomplete(id, {
        ...
        onSearch: ({ currentValue }) => {
            let res = new Promise((resolve) => {
                ...
                else if (window.location.href.indexOf('tokenflowchart') > -1 && isMainPanel === false) {
                    filterBy = 16;
                    inPageSearch = true;
                ...
                fetch(strUrl).then((response) => response.json()).then((data) => {
                    if (data.length !== 0) {
        ...
        onResults: ({ currentValue, matches, template, classGroup }) => {
            ...
            return matches === 0 ? template : matches.map((el, index, array) => {
                let group = (isMainPanel || isAdvancedFilterSearch) && el[attributes.group] !== array[index - 1]?.[attributes.group] ? `<div class="search-category-${el[attributes.group].toLowerCase().replace(/ /g, '-').replace('-(erc-20)', '').replace('-(erc-721-&amp;-1155)', '')} ${classGroup}" style="font-size: 0.65rem;"><span>${el[attributes.group]}</span></div>` : "";
                let img_url = '';
                let altText = '';

                if (el[attributes.groupid] === '3') {
                    if (el[attributes.img] === '') {
                        img_url = '/images/main/empty-token.png';
                    } else {
                ...
                if (el[attributes.groupid] === '2') {
                    img_url = blockies.create({
                        seed: el[attributes.address].toLowerCase(),
                        size: 8,
                        scale: 16
                    }).toDataURL();
                    altText = 'Address';
                ...
                let searchResultLink = el[attributes.link];
                let onSearchResultClick = '';
                if (searchResultLink.indexOf('tokenflowchart') > -1) {
                    searchResultLink = "javascript:void(0);"; // Prevent navigation for token holders bubble link"
                    onSearchResultClick = `onclick="reloadTokenFlow('${el[attributes.address]}', 90);"`
                }
                ...
                return `${group}
                    <li class="nav-item nav-link ${(isMainPanel || isAdvancedFilterSearch) ? 'mb-2' : 'my-2'} rounded" style="cursor: pointer;">
                        ...
                        <a href="${searchResultLink}" ${onSearchResultClick} class="nav-link p-2 d-flex ${el[attributes.address] ? 'align-items-start' : 'align-items-center'} w-100">
                            <div class="me-2">
        ...
        ,
        onSubmit: ({ index }) => {
            if (inPageSearch === true) {
                searchInput.value = searchTerm;

                const dropdownToggle = document.getElementById(id).closest('.dropdown').querySelector('a[data-bs-toggle="dropdown"]');
        ...
        onRender: async ({ results }) => {
            if (isMainPanel || isAdvancedFilterSearch) {
                if (isMainPanel) {
                    results.insertAdjacentHTML('beforebegin', getSeachBoxAd() + `<div class="search-panel-header"></div>`);
                }
            ...
            results.insertAdjacentHTML('beforebegin', `<div class="search-panel-header"></div>`);
        }
        ,
